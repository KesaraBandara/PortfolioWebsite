import logo from "../assets/kesara_logo.png";
import { FaLinkedin } from "react-icons/fa";
import { FaGithub } from "react-icons/fa";
// import { FaSequ } from "react-icons/fa"
// import { FaLinkedin } from "react-icons/fa"

function Navba() {
  return (
    <nav className=" mb-20 flex items-center justify-between py-6">
      <div className="flex flex-shrink-0 items-center animate-pulse">
      <div className="border-2 border-black p-1 rounded-full">
    <div className="border-2 border-white p-1 rounded-full">
        <img className="mx- w-16 rounded-full" src={logo} alt="logo" />
      </div>
      </div>
      </div>
      <div className="m-8 flex items-center justify-center gap-4 text-2xl">

        <a
          href="https://www.linkedin.com/in/harsha-bandara-61575425b/"
          target="_blank"
          rel="noopener noreferrer"
        >
          <FaLinkedin size={30} />
        </a>

        <a
          href="https://github.com/KesaraBandara?tab=repositories"
          target="_blank"
          rel="noopener noreferrer"
        >
          <FaGithub size={30} />
        </a>
      </div>
    </nav>
  );
}

export default Navba;
